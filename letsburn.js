...
const getReward = async () =>{
	...
	await contractInstance.getCurrentReward(2, function (err, res) {
		boaReward = res / 10**18;
		console.log("BOA TEST: " + res / 10**18);
	});

	await contractInstance.getCurrentReward(3, function (err, res) {
		ethReward = res / 10**18;
...
await updateUserStats();
  ...
  switch (bonusAddress) {
    case PAIRS.YFKA_XAMP:
      ...
      $('#pool-XAMP').style.backgroundImage="url(imgs/fireBG3.png)";
      break;
    case PAIRS.YFKA_TOB:
      $('#bonus-global-token').html('TOB');
      $('#tob-apy').html(`${bonusEmissionRate}`);
      $('#pool-TOB').style.backgroundImage="url(imgs/fireBG3.png)";
      break;
    case PAIRS.YFKA_BOA:
      $('#bonus-global-token').html('BOA');
      $('#boa-apy').html(`${bonusEmissionRate}`);
      $('#pool-BOA').style.backgroundImage="url(imgs/fireBG3.png)";
      break;
    case PAIRS.YFKA_ETH:
    default:
      // Dont do shit
